    ...
    for ax, plot_name in zip(axes, components):
        if plot_name == "trend":
            ...
            if (
                ...
                plot_name == "yearly" or m.seasonalities[plot_name]["period"] == 365.25
            ):
                plot_yearly(
                    m=m,
                    name=plot_name,
...
def plot_seasonality(m, name, ax=None, uncertainty=True, figsize=(10, 6)):
    ...
    xticks = pd.to_datetime(
        np.linspace(start.value, end.value, n_ticks)
    ).to_pydatetime()
...
def add_changepoints_to_plot(
    ...
    signif_changepoints = (
        m.changepoints[np.abs(np.nanmean(m.params["delta"], axis=0)) >= threshold]
...
def plot_cross_validation_metric(
    ...
    x_plt = df_none["horizon"].astype("timedelta64[ns]").view(np.int64) / float(
        dt_conversions[i]
    )
    x_plt_h = df_h["horizon"].astype("timedelta64[ns]").view(np.int64) / float(
        dt_conversions[i]
    )
...
def plot_components_plotly(
    ...
    fig["layout"].update(
        go.Layout(
            showlegend=False, width=figsize[0], height=figsize[1] * len(components)
        )
    )
...
def get_seasonality_plotly_props(m, name, uncertainty=True):
    ...
    days = pd.to_datetime(
        np.linspace(start.value, end.value, plot_points, endpoint=False)
    )
    df_y = seasonality_plot_df(m, days)
    seas = m.predict_seasonal_components(df_y)
    ...
    if (
        uncertainty
        and m.uncertainty_samples
        and (seas[name + "_upper"] != seas[name + "_lower"]).any()
    ):
        traces.append(
            go.Scatter(
                name=name + "_upper",
    ...
    yaxis = go.layout.YAxis(
        title=go.layout.yaxis.Title(text=name), zerolinecolor=zeroline_color
    )
    if m.seasonalities[name]["mode"] == "multiplicative":
        yaxis.update(tickformat="%", hoverformat=".2%")
