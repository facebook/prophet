...
class TestProphet(TestCase):
    ...
    def test_fit_predict(self):
        ...
        forecaster = Prophet(stan_backend="NUMPYRO")
    ...
    def test_fit_sampling_predict(self):
        ...
        forecaster = Prophet(mcmc_samples=500, stan_backend="NUMPYRO")
    ...
    def test_fit_predict_no_changepoints(self):
        ...
        forecaster = Prophet(n_changepoints=0, stan_backend="NUMPYRO")
        forecaster.fit(train)
        forecaster.predict(future)
    ...
    def test_fit_changepoint_not_in_history(self):
        ...
        prophet = Prophet(changepoints=['2013-06-06'], stan_backend="NUMPYRO")
        forecaster = prophet
        forecaster.fit(train)
        forecaster.predict(future)
    ...
    def test_fit_predict_constant_history(self):
        ...
        train['y'] = 20
        future = pd.DataFrame({'ds': DATA['ds'].tail(N // 2)})
        m = Prophet(stan_backend="NUMPYRO")
        m.fit(train)
    ...
    def test_fit_predict_uncertainty_disabled(self):
        ...
        for uncertainty in [0, False]:
            m = Prophet(uncertainty_samples=uncertainty, stan_backend="NUMPYRO")
            m.fit(train)
            fcst = m.predict(future)
    ...
    def test_logistic_floor(self):
        m = Prophet(stan_backend="NUMPYRO", growth='logistic')
        N = DATA.shape[0]
        history = DATA.head(N // 2).copy()
        history['floor'] = 10.
        history['cap'] = 80.
        ...
        m2 = Prophet(stan_backend="NUMPYRO", growth='logistic')
    ...
    def test_flat_growth(self):
        m = Prophet(growth='flat', stan_backend="NUMPYRO")
    ...
    def test_invalid_growth_input(self):
        ...
        with self.assertRaisesRegex(ValueError, msg):
            Prophet(growth="constant", stan_backend="NUMPYRO")
    ...
    def test_set_changepoint_range(self):
        m = Prophet(changepoint_range=0.4, stan_backend="NUMPYRO")
        N = DATA.shape[0]
        history = DATA.head(N // 2).copy()
        ...
        with self.assertRaises(ValueError):
            m = Prophet(changepoint_range=-0.1, stan_backend="NUMPYRO")
        with self.assertRaises(ValueError):
            m = Prophet(changepoint_range=2, stan_backend="NUMPYRO")

    def test_get_zero_changepoints(self):
        m = Prophet(n_changepoints=0, stan_backend="NUMPYRO")
        N = DATA.shape[0]
        history = DATA.head(N // 2).copy()
    ...
    def test_growth_init(self):
        model = Prophet(growth='logistic', stan_backend="NUMPYRO")
    ...
    def test_piecewise_linear(self):
        model = Prophet(stan_backend="NUMPYRO")
    ...
    def test_holidays(self):
        ...
        model = Prophet(holidays=holidays, stan_backend="NUMPYRO")
        df = pd.DataFrame({
        ...
        m = Prophet(holidays=holidays, stan_backend="NUMPYRO")
        ...
        m = Prophet(holidays=holidays2, stan_backend="NUMPYRO")
        feats, priors, names = m.make_holiday_features(df['ds'], m.holidays)
        ...
        with self.assertRaises(ValueError):
            Prophet(holidays=holidays, stan_backend="NUMPYRO").make_holiday_features(df['ds'], holidays)
    ...
    def test_fit_with_holidays(self):
        ...
        model = Prophet(holidays=holidays, uncertainty_samples=0, stan_backend="NUMPYRO")
    ...
    def test_fit_predict_with_country_holidays(self):
        ...
        model = Prophet(uncertainty_samples=0, stan_backend="NUMPYRO")
        model.add_country_holidays(country_name='US')
    ...
    def test_auto_weekly_seasonality(self):
        ...
        m = Prophet(weekly_seasonality=True, stan_backend="NUMPYRO")
        m.fit(train)
        self.assertIn('weekly', m.seasonalities)
        ...
        m.fit(train)
        self.assertNotIn('weekly', m.seasonalities)
        m = Prophet(weekly_seasonality=2, seasonality_prior_scale=3., stan_backend="NUMPYRO")
        m.fit(DATA)
        self.assertEqual(
            m.seasonalities['weekly'],
            {
    ...
    def test_auto_yearly_seasonality(self):
        ...
        m.fit(train)
        self.assertIn('yearly', m.seasonalities)
        m = Prophet(yearly_seasonality=7, seasonality_prior_scale=3., stan_backend="NUMPYRO")
        m.fit(DATA)
        self.assertEqual(
            m.seasonalities['yearly'],
            {
    ...
    def test_auto_daily_seasonality(self):
        ...
        m = Prophet(daily_seasonality=True, stan_backend="NUMPYRO")
        m.fit(train)
        self.assertIn('daily', m.seasonalities)
        m = Prophet(daily_seasonality=7, seasonality_prior_scale=3., stan_backend="NUMPYRO")
        m.fit(DATA2)
        self.assertEqual(
            m.seasonalities['daily'],
            {
    ...
    def test_conditional_custom_seasonality(self):
        m = Prophet(weekly_seasonality=False, yearly_seasonality=False, stan_backend="NUMPYRO")
    ...
    def test_set_seasonality_mode(self):
        ...
        m = Prophet(seasonality_mode='multiplicative', stan_backend="NUMPYRO")
    ...
    def test_seasonality_modes(self):
        ...
        m = Prophet(seasonality_mode='multiplicative', holidays=holidays, stan_backend="NUMPYRO")
