...
def data():
    ts = pd.read_sql("select no_of_request from timeseries", settings.engine)
    ts = np.array(ts)

    return ts

def gaussian(ts, p, sp, ahead, mtype, alpha=None, beta=None, gamma=None):

    a, b, s = _initialize(mtype, ts, p, sp)
    ...
    MSD, params, smoothed = _filter(mtype, ts, p, a, b, s[:], alpha, beta, gamma)
    forecasted = _forecast(mtype, p, ahead, params)

    return forecasted


def _initialize(mtype, ts, p,sp):
...
def _MSD(tuning, *args):
    ...
    for t in range(len(ts)):
        ...
        predicted.append(((Lt1 + Tt1) * St1[t % p]).item(0))
...
def _filter(mtype, ts, p, a, b, s, alpha, beta, gamma):
    smoothed = []
    Lt1, Tt1, St1 = a, b, s[:]
    ...
    for t in range(len(ts)):
        ...
        Tt = beta * (Lt - Lt1) + (1 - beta) * (Tt1)
        St = gamma * (ts[t] / Lt) + (1 - gamma) * (St1[t % p])
        smoothed.append(((Lt1 + Tt1) * St1[t % p]).item(0))
...
def _forecast(mtype, p, ahead, params):
    Lt, Tt, St = params
    mtype == 'gaussian'
    return [(Lt.item(0) + (t + 1) * Tt.item(0)) * St[t % p].item(0) for t in range(ahead)]

# Main function 
def forecas(no_of_request, no_of_period):
    results = gaussian(no_of_request, 12, len(no_of_request), no_of_period, mtype='gaussian')
    return results
