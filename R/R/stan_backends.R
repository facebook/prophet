...
check_cmdstanr <- function() {
  ...
  cat(paste0("Using cmdstan version ", cmdstanr_version, "\n"))
}
...
#' Fit the Stan model and extract the fitted parameters in a consistent format.
#'
#' @param args Named list of arguments suitable for the chosen backend.
#' @param backend "rstan" or "cmdstanr".
#' @param type "mcmc" or "optimize".
#'
#' @return A named list containing "stan_fit" (the fitted stan object), "params", and "n_iteration"
#' @keywords internal
.fit <- function(args, backend, type) {
  switch(
    backend,
    "rstan" = .fit_rstan(args, type),
    "cmdstanr" = .fit_cmdstanr(args, type)
  )
}

#' @rdname .fit
.fit_rstan <- function(args, type) {
  model_output <- list()
  if (type == "mcmc") {
...
#' @rdname .fit
.fit_cmdstanr <- function(args, type) {
